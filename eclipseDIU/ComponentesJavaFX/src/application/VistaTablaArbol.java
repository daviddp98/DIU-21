...
public class VistaTablaArbol extends Application {
	...
	final ObservableList<Personaje> masculinos = FXCollections.observableArrayList(new Personaje("Pepito", "Grillo"),
			new Personaje("Bob", "Esponja"), new Personaje("Juan", "Sin Miedo"),
			new Personaje("Perico", "De Los Palotes"));
	final ObservableList<Personaje> femeninos = FXCollections.observableArrayList(new Personaje("Juana", "La Loca"));
	...
	public void start(Stage escenarioPrincipal) {
		try {
			...
			columnaNombre.setCellValueFactory(
					(TreeTableColumn.CellDataFeatures<Personaje, String> parametro) -> new ReadOnlyStringWrapper(
							parametro.getValue().getValue().getNombre()));
			columnaApellidos.setMinWidth(150);
			columnaApellidos.setCellValueFactory(
					(TreeTableColumn.CellDataFeatures<Personaje, String> parametro) -> new ReadOnlyStringWrapper(
							parametro.getValue().getValue().getApellidos()));
